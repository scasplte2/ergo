...
class ParametersSpecification extends ErgoPropertyTest {
  ...
  property("soft fork - w. activation") {
    ...
    val rulesToDeactivate = Seq()
    val sfParams = SoftForkParameters(2, 0, rulesToDeactivate)

    val p: Parameters = Parameters(1, Map(BlockVersion -> 0), None)
    val vr: VotingData = VotingData.empty
    ...
    val expectedParameters2 = Parameters(2, Map(BlockVersion -> 0), Some(sfParams))
    ...
    val wrongParameters2 = Parameters(2, Map(BlockVersion -> 0), None)
